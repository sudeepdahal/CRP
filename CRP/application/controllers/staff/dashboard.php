...
class Dashboard extends CI_Controller{
	...
	public function index()
	{
		...
		//$note['notef']=$this->staff_model->getNotification();
		$note = array('notef'=>$this->staff_model->getNotification(),
					  'sucmsg' => ""
					  );
		...
		if(isset($email) && $type=="staff")
		{
        	...
        	$this->load->view('staff_views/dashboard',$note);
		...
		public function addNotice()
		{
			...
			$notice = array(
							'publish_date' => $date,
   							...
   							'body' => $this->input->post('qbody') ,
   							'type' => $this->input->post('ntype') ,
   							'userid' => $email ,
   							'postby' => $this->input->post('postby')
						);
			//print_r($notice);
			$status = $this->staff_model->addNotice($notice);
		...
		public function updatecode()
		{
			...
			$status = $this->staff_model->updatecode($uid,$sec_code);
